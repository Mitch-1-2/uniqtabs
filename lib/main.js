...
function sortTabsByPinned(sortingTabs)
{
  ...
  for (let i = pinnedStart; i < pinnedTabsLength; ++i) {
    ...
    let similarityThreshold = sortByPinnedThreshold + sizeOfTLD(tokensA);
    let sliceStart = -1;
    let sliceSize = 0;
...
function tabComparator(tabA, tabB)
{
  ...
  let {scheme, host, path} = url.URL(tabA.url || '');
  const schemeA = scheme || '';
  const hostA = host || '';
  const pathA = path || '/';
  const aboutA = scheme === 'about';

  ({scheme, host, path}) = url.URL(tabB.url || '');
  ...
  const aboutPriorityA = aboutA && aboutPaths.get(pathA);
  const aboutPriorityB = aboutB && aboutPaths.get(pathB);
  ...
  if (sortMode !== 2) {
    const [tldA, restA] = partitionHost(hostA.split('.').reverse());
    const [tldB, restB] = partitionHost(hostB.split('.').reverse());
    compResult = tokensComparator(restA, restB);
    if (compResult !== 0)
      return compResult;
    compResult = tokensComparator(tldA, tldB);
    if (compResult !== 0)
      return compResult;
    const aPathless = pathA === '/';
    const bPathless = pathB === '/';
    if (bPathless)
      return +!aPathless;
    if (aPathless)
      return -1;
  ...
  if (sortMode === 1) {
    compResult = tokensComparator(pathA.split('/'), pathB.split('/'));
    if (compResult !== 0)
      return compResult;
  }
  if (true) {
    compResult = (tabA.title || '').localeCompare(tabB.title || '');
    if (compResult !== 0)
      return compResult;
  }
  if (sortMode === 0) {
