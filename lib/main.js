...
function tabComparator(aTab, bTab)
{
  ...
  if (sortByTimeCreated !== 'none') {
    compResult = (aTabProps && aTabProps.get('createTime') || 0) -
                 (bTabProps && bTabProps.get('createTime') || 0);
    m = -(sortByTimeCreated === 'most');
    return compResult * m;
  }
  else if (sortByTimeAccessed !== 'none') {
    compResult = (aTabProps && aTabProps.get('accessTime') || 0) -
                 (bTabProps && bTabProps.get('accessTime') || 0);
    m = -(sortByTimeAccessed === 'most');
    return compResult * m;
  }

  const sortMode = sortModes.get(sortBy);
...
function tabActivated(tab)
{
  ...
  tabMap.get(tab).set('accessTime', dateNow);
}
...
function tabLoaded(tab)
{
  ...
  if (!tabMap.get(tab).get('isNew'))
    return;
  tabMap.get(tab).set('isNew', false);
