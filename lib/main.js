...
function cullBlankTabs(window)
{
  ...
  cullingTimeout = timers.setTimeout(function(window) {
    ...
    for (let i = windowTabsLength - 1; i >= 0; --i) {
      ...
      let browser = tabUtils.getBrowserForTab(lowLevelTab);
      if (browser.contentDocument) {
        // tab.readyState available in Firefox 33+ except when a content
        // document is inaccessible, e.g. about:privatebrowsing
        isReady = ('readyState' in tab) && (tab.readyState === 'interactive' ||
...
function setTabContextMenus(window = null)
{
  ...
  let sortTabs_accesskey = _('tabContextMenu_sortTabs.accesskey');
  const sortTabs_label_key = 'tabContextMenu_sortTabs.label';
  let sortTabs_label = _(sortTabs_label_key);
  if (sortTabs_label == sortTabs_label_key) {
    // Failed to get localised string. Use hard-coded strings instead.
    sortTabs_label = 'Sort Tabs';
    sortTabs_accesskey = 'S';
  }
